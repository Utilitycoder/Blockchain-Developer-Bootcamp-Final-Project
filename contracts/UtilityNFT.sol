...
contract UtilityNFT is ERC721URIStorage {
    ...
    mapping(uint256 => nftItem) private idToUtilityNFT;
    ...
    constructor(address marketplaceAddress) ERC721("UtilityNFT", "UNFT") {
    ...
    function createUtilityNFT() public returns(uint256) {
        ...
        idToUtilityNFT[newItemId] = nftItem(newItemId, msg.sender, finalTokenURI);
    ...
    function getMyUtilityNFT() public view returns(nftItem[] memory) {
        uint totalItemCount = _tokenId.current();
        uint itemCount = 0;
        uint currentIndex = 0;

        for (uint i = 0; i < totalItemCount; i++) {
            if (idToUtilityNFT[i].owner == msg.sender) {
                itemCount += 1;
            }
        }
        ...
        for (uint i = 0; i < totalItemCount; i++) {
            if (idToUtilityNFT[i].owner == msg.sender) {
                uint currentId = i;
                nftItem storage currentItem = idToUtilityNFT[currentId];
